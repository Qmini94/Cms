package kr.co.itid.cms.controller.cms.core.template;
...
public class LayoutController {
    ...
    /**
     * 레이아웃 조회
     * - 관리자 layout을 수정하기 위해 사이트의 타입에 대한 순수 데이터를 조회한다.
     */
    @PreAuthorize("@permService.hasAccess('MODIFY')")
    @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<ApiResponse<LayoutResponse>> get(
            @RequestParam @NotNull @Positive Long siteIdx,
            @RequestParam @NotNull LayoutKind kind
            ) throws Exception {
        LayoutResponse data = layoutService.getTemplateBySiteIdxAndKind(siteIdx, kind);
